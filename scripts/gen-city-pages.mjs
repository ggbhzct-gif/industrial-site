...
const SERVICES_HUB = [
  {
    ...
    desc: "长三角及安徽区域废旧电缆上门评估结算：铜芯 YJV / 铝芯 YJLV / 10kV·35kV 高压交联电缆 / 低压电力电缆 / 控制电缆 / 电机引出线。按净铜铝 × 当日长江有色金属价 × 回收系数结算，现场过磅，正规单据。",
    kw: [
...
function renderCity(c) {
  ...
  const desc  = `${c.name}废旧电缆回收、二手变压器回收与出售、工厂拆除设备处理。覆盖${c.areas.slice(0,4).join("、")}等${c.name}主要工业区，上门评估，现场过磅，公式计价，电话${PHONE}。`;
  const kw    = [
...
<main>
  <section class="hero"><div class="wrap">
    ...
    <p>新兴电力设备深耕${c.prov}${c.name}区域，为本地工厂、物业、电气承包商提供废旧电缆回收、二手变压器回收与出售、整厂拆除设备处理。覆盖${c.areas.join("、")}等${c.name}主要工业区，免费上门评估、公式计价、现场称重结算。</p>
    <div>
